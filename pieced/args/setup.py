...
from pieced.args.dataset import augmentations_args, dataset_args
from pieced.args.utils import additional_setup_linear, additional_setup_pretrain
from pieced.args.continual import continual_args
...
def parse_args_pretrain() -> argparse.Namespace:
    ...
    # parser.add_argument("--task_reverse", action="store_true", help="Reverse the order of tasks")
    parser.add_argument("--task_order", type=str, default=None, help="커스텀 Task 순서 지정 (예: '1,2,3,4,5' 또는 '3,1,5,2,4')")
    parser.add_argument("--task_split_json", type=str, default="./task_orders.json",
                        help="JSON file containing class-to-task assignments")
    parser.add_argument("--task_split_key", type=str, default=None,
                        help="Top-level key in task_split_json (e.g. 'NTU60_xsub'). "
                             "If unset, falls back to legacy randperm(seed=5) splitting.")
    temp_args, _ = parser.parse_known_args()
